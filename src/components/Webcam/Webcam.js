...
const WebcamStreamCapture = () => {
	...
	return (
		<>
			...
			<div style={{ width: "100%" }}>
				{capturing ? (
					<button style={{ width: minValue }} onClick={handleStopCaptureClick}>
						Stop Capture
					</button>
				) : (
					<button style={{ width: minValue }} onClick={handleStartCaptureClick}>
						Start Capture
					</button>
				)}
			</div>
			...
			<p>
				the size of the windo is {size.height} x {size.width}
			</p>
			<p>
				camera width = {cameraWidth} and cameraHeight = {cameraHeight}
			</p>
